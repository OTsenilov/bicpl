    ...
    Real             *dist )
{
    ...
    x_dot_p = rx * v01x + ry * v01y + rz * v01z;
    y_dot_p = rx * v02x + ry * v02y + rz * v02z;

    sd = (x_dot_p * y_dot_y - x_dot_y * y_dot_p) / bottom;
    td = (y_dot_p * x_dot_x - x_dot_y * x_dot_p) / bottom;

    nx = rx - sd * v01x - td * v02x;
    ny = ry - sd * v01y - td * v02y;
    nz = rz - sd * v01z - td * v02z;

    len_n = nx * nx + ny * ny + nz * nz;
    if( len_n == 0.0 )
        return( FALSE );
    ...
    d = (nx * dx + ny * dy + nz * dz) / len_n;

    x_dot_p = dx * v01x + dy * v01y + dz * v01z;
    y_dot_p = dx * v02x + dy * v02y + dz * v02z;

    so = -(x_dot_p * y_dot_y - x_dot_y * y_dot_p) / bottom;
    to = -(y_dot_p * x_dot_x - x_dot_y * x_dot_p) / bottom;
